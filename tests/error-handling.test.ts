...
import { createTask, deleteTask, getTasks } from '../src/tools/handlers/tasks.js';
...
describe('Error Handling', () => {
  ...
  describe('Authentication Errors', () => {
    ...
    it('handles invalid credentials', async () => {
      ...
      await expect(createTask({ type: 'todo', text: 'Test' })).rejects.toThrow(
  ...
  describe('Validation Errors', () => {
    it('handles 400 bad request with validation errors', async () => {
      ...
      await expect(createTask({ type: 'invalid' as unknown, text: 'Test' })).rejects.toThrow(
        'Invalid task type',
      );
    });
  });
